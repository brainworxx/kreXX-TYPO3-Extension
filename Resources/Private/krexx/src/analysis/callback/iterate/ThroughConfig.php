...
class ThroughConfig extends AbstractCallback
{
    ...
    public function callMe()
    {
        ...
        foreach ($sections as $sectionName => $sectionData) {
            ...
            $configOutput .= $this->pool->render->renderExpandableChild($model);
        }
        // Render the dev-handle field.

        $data = 'Local open function';
        $editableModel = $this->pool->createClass('Brainworxx\\Krexx\\Analyse\\Model')
            ->setData($data)
        ...
        $configOutput .= $this->pool->render->renderSingleEditableChild($editableModel);
        // Render the reset-button which will delete the debug-cookie.
        $buttonModel = $this->pool->createClass('Brainworxx\\Krexx\\Analyse\\Model')
            ->setName('resetbutton')
            ->setNormal('Reset local settings')
            ->setHelpid('resetbutton');

        $configOutput .= $this->pool->render->renderButton($buttonModel);
        return $configOutput;
    }
}
